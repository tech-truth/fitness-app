...
export function WorkoutTemplateList() {
  ...
  useEffect(() => {
    ...
    async function loadTemplates() {
      ...
      setState({
        templates: ((data ?? []) as unknown as WorkoutTemplateRow[]).map(formatWorkoutTemplate),
        isLoading: false,
        error: null,
      });
    }
  ...
  return (
    <TemplateShell
      ...
      description="Reusable training plans ready to assign to clients."
      createHref="/templates/workouts/new"
      createLabel="New workout"
    >
      {state.error ? (
      ...
      {state.isLoading ? (
            ...
            <article key={template.id} className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-2 text-sm text-zinc-500">{template.goal}</p>
                </div>
              ...
              <div className="mt-5 grid grid-cols-3 gap-3">
                <Metric label="Duration" value={template.duration} />
                <Metric label="Days" value={String(template.daysCount)} />
                <Metric label="Exercises" value={String(template.exercisesCount)} />
              </div>
        ...
        <EmptyTemplates
          ...
          description="Create your first reusable workout plan, then assign it from a client profile."
...
function formatWorkoutTemplate(template: WorkoutTemplateRow): WorkoutTemplateCard {
  const days = template.workout_days ?? [];
  const exercisesCount = days.reduce((total, day) => total + (day.workout_exercises?.length ?? 0), 0);

  return {
    id: template.id,
    name: template.name ?? "Untitled workout",
    goal: formatLabel(template.goal ?? "Not set"),
    difficulty: formatLabel(template.difficulty ?? "Not set"),
    duration: template.duration_weeks ? `${template.duration_weeks} weeks` : "Not set",
